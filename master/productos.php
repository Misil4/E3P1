...
if ($num_total_rows > 0) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo '</div>';}
        echo '</ul>';}
    echo '<nav style="left: 33%;position:relative">';
    echo '<ul class="pagination">';
    if ($total_pages > 1 && $_GET['Filtro']['Activo']!= "ok") {
        if ($page != 1) {echo '<li class="page-item"><a class="page-link" href="productos?page=' . ($page - 1) . '"><spanaria-hidden="true">&laquo;</span></a></li>';}

        for ($i = 1; $i <= $total_pages; $i++) {if ($page == $i) {echo '<li class="page-item active"><a class="page-link" href="#">' . $page . '</a></li>';} 
        else {echo '<li class="page-item"><a class="page-link" href="productos?page=' . $i . '">' . $i . '</a></li>';}}
                
        if ($page != $total_pages){echo '<li class="page-item"><a class="page-link" href="productos?page=' . ($page + 1) . '"><span aria-hidden="true">&raquo;</span></a></li>';}}
    echo '</ul>';
    echo '</nav>';}?>
                            </div>
                        </div>
