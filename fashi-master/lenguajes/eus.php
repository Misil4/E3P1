<?php

$lang = array(

    "titulo" => "K-Beauty Zure kosmetika koreak denda",
    "login" => "Saioa hasi",
    "Miembros" => "Kideak",
    "Limpiadores" => "Garbitzaileak",
    "Exfoliantes" => "Sastrakak",
    "Tónicos" => "Tonikoak",
    "Esencias" => "Esentziak",
    "Serums" => "Serums",
    "Mascarillas" => "Maskarak",
    "Parches" => "Adabakiak",
    "Hidratantes ojos" => "Begi hidratatzaileak",
    "Crema Solar" => "Eguzkitako krema",
    "Mascarillas de noche y bb cream" => "Gaueko maskarak eta bb krema",
    "Men" => "Gizona",
    "Women" => "Emakumea",
    "categories_description" => "Bilatu",
    "total" => "Totala:",
    "cart_view" => "SASKIA IKUSI",
    "check_out" => "EROSKETA AMAITU",
    "eslogan" => "EROSI ORAIN",
    "Inicio" => "Hasiera",
    "Productos" => "Produktuak",
    "Marcas" => "Markak",
    "Blog" => "Bloga",
    "Precios" => "Prezio",
    "Contacto" => "Kontaktua",
    "Descripcion" => "Questa è la descrizione del mio sito Web Italiano",
    "Titular" => "Il mio sito Web in Italiano",
    "en" => '<img src="img/flag-1.jpg"></img>',
    "es" => '<img src="img/flag-4.png" width="25px"></img>',
    "eus" => '<img src="img/flag-3.png"></img>',
    // Index

    "Oferta" => "Beherapena",
    "Ver mas" => "Gehiago ikusi",

    //login

    "Email" => "Posta Elektronikoa",
    "Password" => "Pasahitza",
    "Save Password" => "Pasahitza Gorde",
    "Lost Password" => "Pasahitza Berreskuratu",
    "Or Create An Account" => "Edo Kontu Bat Sortu",

    //register

    "Register" => "Erregistroa",
    "Password Confirm" => "Pasahitza Baieztatu",
    "Or Login" => "Edo Saioa Hasi",
    //  Area de Contacto

    "Contacta con nosotros" => "Gurekin harremanetan jarri",
    "Contacto" => "Kontaktu",
    "Telefono" => "Telefono-zenbakia",
    'Deja tu comentario' => 'Uzi zure iritzia',
    'Comentario explicación' => 'Atal honetan zure zalantzak edo iradokizuak utz ditzakezu',
    'Comentario explicación nombre' => 'Sartu zure izena',
    'Comentario explicación email' => 'Sartu zure abizena',
    'Comentario explicación mensaje' => 'Sartu zure mezua',
    'Enviar Mensaje' => 'Mezua bidali',

    //general
    "en" => '<img src="img/flag-1.jpg"></img>',
    "es" => '<img src="img/flag-4.png" width="25px"></img>',
    "eus" => '<img src="img/flag-3.png"></img>',
);